...
try:
    ...
    _logger.error("Can not set a correct cipher.")
...
class Invoice(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == "div":
            ...
            if (
                "id" in attrib
                and attrib["id"] == self.NOT_RECORDS
            ...
            if attrib.get("id", "") == "BtnRecuperaAcuse":
                self._last_request_link_pdf = attrib["onclick"].split("'")[1]
            if attrib.get("id", "") == "BtnRecuperaAcuseFinal":
    ...
    def handle_endtag(self, tag):
        if self._is_div_page and tag == "tr":
            if self._last_uuid:
                ...
                url_request_pdf = ""
                if self._last_request_link_pdf:
                    url_request_pdf = "{}{}".format(
                        self.URL, self._last_request_link_pdf
                    )
                ...
                invoice = (
                    self._last_uuid,
                    {
                        "url": url_xml,
                        "acuse": url_pdf,
                        "url_request_pdf": url_request_pdf,
...
class PortalSAT:
    ...
    URL_CANCELACION = URL_PORTAL + "AcuseSolicitudCancelacion.aspx"
        ...
        try:
            if method == "get":
                ...
                result = self._session.post(
                    url, data=data, timeout=TIMEOUT, verify=VERIFY_CERT
                )
    ...
    def login(self, ciec, from_script):
        ...
        data = self._read_form(
            self._response("https://cfdiau.sat.gob.mx/nidp/wsfed/ep?sid=0", data=data)
        )
        data = self._read_form(
            self._response(
    ...
    def _get_data_cert(self, fiel_cert_data):
        ...
        serie = "{:x}".format(cert.get_serial_number())[1::2]
    ...
    def _sign(self, fiel_pem_data, data):
        ...
        sign = base64.b64encode(crypto.sign(key, data.encode("utf-8"), "sha256"))
    ...
    def login_fiel(self, fiel_cert_data, fiel_pem_data):
        ...
        result = self._session.get(self.URL_MAIN, verify=False)
    ...
    def _get_filters(self, args, emitidas=True):
        ...
        if args["fecha_inicial"] and args["fecha_final"]:
            ...
            data["date_from"], data["date_to"] = self._time_delta(
                args["intervalo_dias"]
            )
            filters.append(Filters(data))
    ...
    def _search_by_uuid(self, filters):
        for f in filters:
            ...
            if not_found:
                _logger.info("-> 0 documentos por descargar en: %s" % str(f))
            else:
    ...
    def _thread_download(self, invoices, folder, filters):
        ...
        for i in range(TRY_COUNT):
            for uuid, values in for_download:
                ...
                # paths[uuid] = path_xml
                content = self._get_link(uuid, values["url"], current, total)
                if content:
                    content_pdf = False
                    if values["acuse"] != '':
                        content_pdf = self._get_link(uuid, values["acuse"], current, total)
                    content_pdf_req = False
                    if values["url_request_pdf"] != '':
                        content_pdf_req = self._get_link(
                            uuid, values["url_request_pdf"], current, total
                        )
                    invoice_content.update(
                        {uuid: [values, content, content_pdf, content_pdf_req]}
                    )
                current += 1
        ...
        if total:
            _logger.info(
                "-> %s documentos por descargar en: %s" % (total, str(filters))
            )
        return invoice_content

    def _get_link(self, uuid, url, current, count):
        for i in range(TRY_COUNT):
            try:
    ...
    def logout(self):
        _logger.debug("Cerrando sessión en el SAT")
        self._response(self.URL_LOGOUT)
        self.is_connect = False
        _logger.info("Sesión cerrada en el SAT")
        return
